defmodule Stripe.Error do
  ...
  - `:source` – this is one of
    ...
    * `:network` – the error occurred while making the network request (i.e. `Req.request/1`
      returned an error.) In this case, `:code` will always be `:network_error`. The
      `:network_reason` field in the `:extra` map contains the actual exception received from
      the HTTP client.
  ...
  - `:network_reason` – for `:network` errors, contains the exception received from the HTTP client.
  ...
  @type t :: %__MODULE__{
          ...
          extra: %{
            ...
            optional(:network_reason) => any
          }
        }

  @enforce_keys [:source, :code, :message]
  defexception [:source, :code, :request_id, :extra, :message, :user_message]
  ...
  def from_network_error(reason) do
    %__MODULE__{
      ...
      code: :network_error,
      message:
        "An error occurred while making the network request. The HTTP client returned the following reason: #{describe(reason)}",
      extra: %{
